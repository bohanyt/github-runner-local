...
| D25 | ACCEPTED | Owner authorizes Checkpoint C **source-only** implementation per GRL-009 (#10): injected/mockable GitHub device-flow/REST adapter, reviewed runner pin/download/hash/safe-extraction pipeline, and runner CLI capability contracts with local Windows contract probe. No GitHub App creation/visibility choice, live login, execution repo, runner registration/start, D/E activation, hosted Actions, service/UAC or release work. Source: Issue #1 comment `5811504614`. |
| D26 | ACCEPTED | Owner authorizes Checkpoint E **source/template-only** implementation per GRL-010 (#11) in parallel with C: inactive nested execution-repo workflow/local-JS-action/profile/fixture source, Node tests and template lint only. No private execution repo creation/use, template activation, root workflow changes, Actions dispatch, runner activation, Stage-2 credentials, D/G1/service/release work. Source: Issue #1 comment `5811700793`. |
| D27 | ACCEPTED | E exact-SHA pre-execution mismatch uses a template-only negative report, not `grl.result.v1`: zero profile execution, no fabricated `tested_sha`, no canonical result, explicit `grl-exec-refusal v1` comment + failing commit status + failing verdict. Complete refusal reporting yields terminal BLOCKED; partial/missing refusal publication yields REPORTING_INCOMPLETE. Root Core/schema remain unchanged. Source: Issue #11 CT clarification `5812014790`. |
| D28 | ACCEPTED | E race reconciliation is no-force and evidence-driven: compare stale post-revocation cloud head `e13f868...` against unpublished local `dbba8b3...`; select/integrate the stronger compliant tree only after full proof. If local/integrated tree is selected, preserve both lineages with a fast-forward reconciliation merge whose first parent is current remote E head, second parent is selected local commit, and tree exactly equals selected local tree. Source: Issue #11 CT packet `5823097901`. |
