...
Results and status publications retry independently, at most three attempts. The observer distinguishes complete E-B1 refusal from interrupted or partially reported runs. Reconciliation is bounded to the recent mailbox window. GitHub Actions runtime proof, especially report-only rerun behavior after an interrupted run, remains an activation prerequisite and is **not** claimed by these local source checks.
