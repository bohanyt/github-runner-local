...
try {
  ...
  const execution = await runProfile({
    ...
    runner: {
      ...
      version: process.env.GRL_RUNNER_VERSION,
      identityClass: process.env.GRL_IDENTITY_CLASS
    }
  });
  const outcomeDir = resolve(process.env.GITHUB_WORKSPACE, 'grl-outcome');
  mkdirSync(outcomeDir, { recursive: true });
  writeFileSync(resolve(outcomeDir, 'identity.json'), JSON.stringify(identity), 'utf8');
  if (execution.kind === 'refusal') {
    writeFileSync(resolve(outcomeDir, 'outcome.json'), JSON.stringify(execution.outcome), 'utf8');
    writeFileSync(resolve(outcomeDir, 'refusal.marker'), 'CHECKOUT_SHA_MISMATCH\n', 'utf8');
    output('result_b64', '');
    output('outcome_b64', encoded(execution.outcome));
    output('execution_status', 'BLOCKED');
  } else {
    writeFileSync(resolve(outcomeDir, 'result.json'), execution.resultJson, 'utf8');
